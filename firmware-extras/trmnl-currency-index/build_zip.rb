...
#
# Usage:
#   ruby firmware-extras/trmnl-currency-index/build_zip.rb                # build ZIP only
#   ruby firmware-extras/trmnl-currency-index/build_zip.rb upload <HOST>  # build + curl upload
#
# Example:
#   ruby firmware-extras/trmnl-currency-index/build_zip.rb upload 45.76.179.84:2300
...
OUT_ZIP  = File.join(OUT_DIR, "currency-index.zip")
...
# TRMNL store → Terminus local-import mapping (see vn-stock build_zip.rb for rationale).
label = trmnl.fetch("name", "Currency Index")
...
# Terminus Types::Version regex: /\A\d+\.\d+\.\d+\Z/  → must be strict semver.
...
config = {
  ...
  "name"              => name,
  "label"             => label,
  "description"       => "USD-index style table of base-vs-symbols rates from Frankfurter (ECB free API). Pure HTML/CSS, no JS.",
  "mode"              => PLUGIN_MODE,
  "kind"              => "poll",
  "tags"              => ["finance", "fx", "currency"],
  "static_body"       => {},
  "fields"            => Array(trmnl["custom_fields"]),
  "data"              => {},
  "interval"          => Integer(trmnl.fetch("refresh_interval", 5)),
  "unit"              => "minute",
  "days"              => [],
