...
#
# Usage:
#   ruby firmware-extras/trmnl-vnd-rates/build_zip.rb                # build ZIP only
#   ruby firmware-extras/trmnl-vnd-rates/build_zip.rb upload <HOST>  # build + curl upload
...
OUT_ZIP  = File.join(OUT_DIR, "vnd-rates.zip")
...
label = trmnl.fetch("name", "VND Rates")
...
config = {
  ...
  "name"              => name,
  "label"             => label,
  "description"       => "Tỷ giá ngoại tệ Vietcombank (mua tiền mặt / mua chuyển khoản / bán). Cập nhật hàng ngày, JSON API, no key.",
  "mode"              => PLUGIN_MODE,
  "kind"              => "poll",
  "tags"              => ["finance", "fx", "vietnam", "vietcombank"],
  "static_body"       => {},
