...
const WATCHED = [
  { key: "sjc_1l", label: "SJC 1L", match: /^Miếng SJC \(Loại 1L\)/i },
  { key: "nu_trang_9999", label: "Nữ Trang 9999", match: /^Nữ Trang 9999/i },
  {
    key: "nhan_9999",
    label: "Nhẫn 9999",
    match: /^NHẪN TRÒN 9999 HƯNG THỊNH VƯỢNG/i,
  },
];
...
function parseXml(xml) {
  ...
  const rows = [];
  const rowRe = /<Row\s+Name='([^']+)'\s+Key='([^']+)'\s+Sell='([^']+)'\s+Buy='([^']+)'\s*\/>/g;
  let m;
